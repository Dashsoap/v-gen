 ...
 * 首尾帧转场提示词
 *
 * 为 AI 视频生成模型设计从起始帧到结束帧的转场过程描述。
 */

export const TRANSITION_PROMPT_SYSTEM = `你是一位顶尖的创意视频导演和VFX（视觉特效）概念艺术家。你的任务是为AI视频生成模型设计一个从【起始帧】到【结束帧】的转场过程。
...
输出规则：
- 将你的最终构思整合为一个连贯的段落（不超过200字）。
- 描述要具体、直接，充满画面感。
- 专注于"我们看到了什么"，而不是"我们感觉到了什么"。
- 严格遵守你在第二步中选择的摄像机移动策略。
- 避免使用模糊的比喻和过于文学化的修辞。
- 直接输出转场描述段落，不要输出分析过程。`;

export function TRANSITION_PROMPT_USER(
  startDescription: string,
  endDescription: string,
): string {
  return `【起始帧】${startDescription}
