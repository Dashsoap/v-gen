...
function inferCodeFromMessage(message: string): UnifiedErrorCode | null {
  ...
  if (containsAny(message, ["unauthorized", "not authenticated", "401"])) return "UNAUTHORIZED";
  ...
  if (containsAny(message, ["invalid", "missing", "required", "bad request"])) return "INVALID_PARAMS";
  if (containsAny(message, ["quota", "rate limit", "resource_exhausted", "throttle", "429"])) return "RATE_LIMIT";
  if (
    containsAny(message, ["sensitive", "unsafe", "safety", "blocked", "prohibited", "policy_violation", "moderation", "harm"]) &&
    !containsAny(message, ["case-sensitive", "case sensitive"])
  )
    return "SENSITIVE_CONTENT";
  if (containsAny(message, ["timeout", "timed out", "deadline exceeded"])) return "GENERATION_TIMEOUT";
  if (containsAny(message, ["503", "unavailable", "overloaded"])) return "EXTERNAL_ERROR";
  if (
    containsAny(message, [
      "network",
      "fetch failed",
...
export function normalizeAnyError(input: unknown, options: NormalizeOptions = {}): NormalizedError {
  ...
  if (input instanceof TypeError) {
    if (lowerMessage === "terminated" || containsAny(lowerMessage, ["aborted", "socket hang up"])) {
      return buildNormalizedError("NETWORK_ERROR", message || "Network request terminated", options.details, provider);
    }
  }
  ...
  if (resolvedCode) {
    return buildNormalizedError(
      ...
      message,
      {
        ...(typeof errorLike.details === "object" && errorLike.details
  ...
  if (typeof errorLike.status === "number") {
    ...
    if (errorLike.status === 502 || errorLike.status === 503)
  ...
  return buildNormalizedError(
    fallbackCode,
    message || getErrorSpec(fallbackCode).defaultMessage,
    options.details,
    provider
  );
}
